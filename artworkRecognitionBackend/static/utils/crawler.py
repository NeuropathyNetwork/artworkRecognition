...
def fetch_baike_info(key_word):
    ...
    try:
        ...
        dt_tag = soup.find('dt', class_='basicInfoItem_NiU_G itemName_Tk1oH', string='出生日期')
        if dt_tag:
            dd_tag = dt_tag.find_next_sibling('dd', class_='basicInfoItem_NiU_G itemValue_IEtvZ')
            if dd_tag:
                span_tag = dd_tag.find('span', class_='text_QZdcK')
                if span_tag:
                    result["birth_date"] = span_tag.get_text(strip=True)

        dt_tag = soup.find('dt', class_='basicInfoItem_NiU_G itemName_Tk1oH', string='逝世日期')
        if dt_tag:
            dd_tag = dt_tag.find_next_sibling('dd', class_='basicInfoItem_NiU_G itemValue_IEtvZ')
            if dd_tag:
                span_tag = dd_tag.find('span', class_='text_QZdcK')
        ...
        # 获取代表作品
        dt_tag = soup.find('dt', class_='basicInfoItem_NiU_G itemName_Tk1oH', string='代表作品')
        if dt_tag:
            dd_tag = dt_tag.find_next_sibling('dd', class_='basicInfoItem_NiU_G itemValue_IEtvZ')
            if dd_tag:
                span_tags = dd_tag.find_all('span', class_='text_QZdcK')
                for span in span_tags:
                    if span.get_text(strip=True) != "，":
                        text = span.get_text(strip=True)
                        if text.endswith("等"):
                            text = text[:-1]
                        result["works"].append(text)
